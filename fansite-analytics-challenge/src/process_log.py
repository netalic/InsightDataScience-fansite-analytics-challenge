...
output_resources = open(sys.argv[3],'a')
output_hours = open(sys.argv[4],'a')
...
def range_time(start, end, delta):
    curr = start
    while curr < end:
        yield curr
        curr += delta
...
time_list = []
for result in range_time (start_date, end_date, timedelta(hours = 1)):
        time_list.append(result)
        
def busiest_60_min_site(site_list, date_time_list):
    counts = {}
    for time in range(len(time_list)):
        start_interval = time_list[time]
        stop_interval = time_list[time] + timedelta(hours = 1)
        list_site = []
        for item in range(len(site_list)):
            date_and_time = datetime.strptime(date_time_list[item], "%d/%b/%Y:%H:%M:%S %z")
            if (date_and_time <= start_interval) and (date_and_time > stop_interval):
                break
            else:
                list_site.append(site_list[item])
        busy = {}
        for items in range(len(list_site)):
            if list_site[items] not in busy:
                busy[list_site[item]] = 1
            else:
                busy[list_site[item]] += 1
        max_frequency = max([i for i in busy.values()])
        time_string = time_list[time].strftime('%d/%b/%Y:%H:%M:%S %z')
        counts[time_string] = max_frequency
...
busiest_60_min_site(site,date_time)
